...
function Shop({ cart, onUpdate, user, setIsLoginOpen }) {
  ...
  // Shop.jsx 中的加入購物車功能
    const handleAddToCart = async (product) => {
        const token = localStorage.getItem('token');  //1. 抓取這名用戶的專屬通行證
        if (!token) return setIsLoginOpen(true);    // 2. 如果沒登入，強制跳出登入視窗（符合邏輯！

        try {
            // 呼叫 API 更新後端資料庫
            await axios.post("http://localhost:3000/api/cart", 
            { product_id: product.id, quantity: 1 },  // 3. 傳入商品編號
            { headers: { Authorization: `Bearer ${token}` } } // 4. 直接把通行證(Token)塞在車頭
            );
                ...
                <div className="action-area">
                  {!item ? (
                    ...
                    <div className="qty-control">
                      ...
                      <input 
                        type="number" 
                        // 當數量是 0 時顯示空字串，方便顧客直接打字
                        value={item.quantity === 0 ? "" : item.quantity}                       
                          
                        onChange={(e) => {
                          const val = e.target.value;
                          if (val === "") {
                            // 1. 當顧客把字刪光時，我們先讓前端狀態暫時變成 0，這樣鍵盤才刪得掉字！
                            updateQty(item.id, 0);
                          } else {
                        ...
                        // 💡 核心優化：當顧客打完字、滑鼠點到旁邊（失去焦點）時觸發
                        onBlur={() => {
                          // 3. 顧客把字刪光（變 0）然後滑鼠點到旁邊，代表他不要了！
                          // 這裡直接執行 updateQty(item.id, 0)，觸發刪除 API，按鈕一秒變回 Add to Cart！
                          if (item.quantity <= 0) {
                            updateQty(item.id, 0);
                          }
                        }}
